	...
	namespace FinalFrontier
	{
		public class HallOfFame : IEnumerable<HallOfFameEntry>
		{
			...
			class DefaultSorter : Sorter<HallOfFameEntry>
			{
				...
				{
					list.Sort(
						delegate (HallOfFameEntry left, HallOfFameEntry right) { return left.GetName().CompareTo(right.GetName()); });
				}
			}
          ...
          */
			...
			public void DumpStatistics()
			{
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					...
					foreach (HallOfFameEntry entry in entries)
					{
						...
						Log.Detail(name + " has " + entry.GetRibbons().Count + " ribbons, " + entry.MissionsFlown + " missions flown in " +
								   entry.TotalMissionTime + " seconds time");
			...
			public void RecordMissionFinished(ProtoCrewMember kerbal)
			{
				...
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					Log.Detail("mission end recorded for kerbal " + kerbal.name + ": " + entry.GetName() + " at " +
							   Utils.ConvertToKerbinTime(now));
				}
			...
			public void RecordLaunch(ProtoCrewMember kerbal)
			{
				...
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					Log.Detail("launch recorded for kerbal " + kerbal.name + ": " + entry.GetName() + " at " +
							   Utils.ConvertToKerbinTime(now));
				}
			...
			public void RecordEva(ProtoCrewMember kerbal, Vessel fromVessel)
			{
				...
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					Log.Detail("EVA recorded for kerbal " + kerbal.name + ": " + entry.GetName() + " at " +
							   Utils.ConvertToKerbinTime(now));
				}
			...
			public void RecordBoarding(ProtoCrewMember kerbal)
			{
				...
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					Log.Detail("vessel boarding recorded for kerbal " + kerbal.name + ": " + entry.GetName() + " at " + now +
							   ", eva time was " + Utils.ConvertToKerbinTime(now - entry.TimeOfLastEva));
				}
			...
			public void RecordDocking(ProtoCrewMember kerbal)
			{
				...
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					Log.Detail("docking recorded for kerbal " + kerbal.name + ": " + entry.GetName() + " at " +
							   Utils.ConvertToKerbinTime(now));
				}
			...
			public void RecordContract(ProtoCrewMember kerbal)
			{
				...
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					Log.Detail("completed contract recorded for kerbal " + kerbal.name + ": " + entry.GetName() + " at " +
							   Utils.ConvertToKerbinTime(now));
				}
			...
			public void RecordScience(ProtoCrewMember kerbal, double science)
			{
				...
				if (Log.IsLogable(Log.LEVEL.DETAIL))
				{
					Log.Detail(science.ToString("00") + " science recorded for kerbal " + kerbal.name + ": " + entry.GetName() + " at " +
							   Utils.ConvertToKerbinTime(now));
				}
			...
			public void Record(ProtoCrewMember kerbal, Ribbon ribbon)
			{
				...
				if (entry == null)
				{
					Log.Error("failed to record ribbon " + ribbon.GetName() + " for kerbal " + kerbal.name + "(type=" + kerbal.type +
							  "), because there is no entry in the hall of fame");
				...
				if (!achievement.HasToBeFirst() || !accomplished.Contains(achievement))
				{
					if (entry.Award(ribbon))
					{
						if (Log.IsLogable(Log.LEVEL.INFO) || FinalFrontier.Config.logRibbonAwards)
						{
							...
							Debug.Log("FF: ribbon " + ribbon.GetName() + " awarded to " + kerbal.name + " at " + time + " (" +
									  Utils.ConvertToKerbinTime(time) + ")");
						}
          ...
          */
			...
			private bool InSameTransaction(LogbookEntry a, LogbookEntry b)
			{
				...
				if (a.UniversalTime > b.UniversalTime + MAX_TIME_DIFF || a.UniversalTime < b.UniversalTime - MAX_TIME_DIFF)
					return false;
				if (b.UniversalTime > a.UniversalTime + MAX_TIME_DIFF || b.UniversalTime < a.UniversalTime - MAX_TIME_DIFF)
					return false;
				return true;
			}
