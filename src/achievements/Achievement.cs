...
namespace Nereid
{
	namespace FinalFrontier
	{
		public abstract class Achievement : Activity, IComparable<Achievement>
		{
			...
			public bool Check(ProtoCrewMember kerbal, ProtoCrewMember.RosterStatus oldState,
			...
			protected virtual bool CheckKerbalRosterStatus(ProtoCrewMember kerbal, ProtoCrewMember.RosterStatus oldState,
				ProtoCrewMember.RosterStatus newState)
			{
				return false;
			}
		...
		class MissionTimeAchievement : NumericAchievement
		{
			...
			public override string GetDescription()
			{
				return "Awarded for more than " + Utils.GameTimeInDays(value) + (GameUtils.IsKerbinTimeEnabled() ? " kerbin" : "") +
					   " days spent in missions";
			}
		}
		...
		class SingleMissionTimeAchievement : NumericAchievement
		{
			...
			{
				return "Awarded for more than " + Utils.GameTimeInDays(value) + (GameUtils.IsKerbinTimeEnabled() ? " kerbin" : "") +
					   " days spent in a single mission and returnig safely";
			}
		}
		...
		class EvaGroundAchievement : CelestialBodyAchievement
		{
			...
			{
				if (current == null) return false;
				if (previous != null && previous.IsEVA && previous.IsLanded)
					return false; // just check first EVA event that was no LANDING
								  // no EVA no ribbon...
				if (!current.IsEVA) return false;
				// on the right place?
				if (!current.MainBody.Equals(body)) return false;
		...
		class CloserSolarOrbitAchievement : CelestialBodyAchievement
		{
			...
			public override string GetDescription()
			{
				return "Awarded for" + FirstKerbalText().Envelope() + "orbiting " + base.body.GetName() + " half between " +
					   innermost.GetName() + " and " + base.body.GetName();
			}
		}
		...
		class HighGeeForceAchievement : NumericAchievement
		{
			...
			public override string GetDescription()
			{
				return "Awarded for withstanding an acceleration of at least " + value + "g for " +
					   GeeForceInspector.DURATION.ToString("0.0") + " or more seconds";
			}
		}
		...
		class MachNumberAchievement : NumericAchievement
		{
			...
			public override string GetDescription()
			{
				return "Awarded for flying horizontally at mach " + value + " below " + MAX_ALTITUDE.ToString("0") +
					   "m in Kerbin atmosphere";
			}
		}
		...
		class GrandTourAchievement : Achievement
		{
			...
			public override string GetDescription()
			{
				return "Awarded for" + FirstKerbalText().Envelope() +
					   "entering the sphere of influence of all celestial bodies in the kerbal system";
			}
		}
		...
		class DeepSpaceAchievement : Achievement
		{
				...
				: base("DS" + (first ? "1" : ""), "Deep Space", prestige, first)
			{
				...
				if (outermostBodyInSystem != null)
				{
					...
					Log.Detail("outermost planet for deep space ribbon is " + outermostBodyInSystem.name + ", min ApA is " +
							   minDistance);
				}
				else
				{
			...
			public override string GetDescription()
			{
				...
				return "Awarded for" + FirstKerbalText().Envelope() + "in space beyond the sphere of influence of " +
					   outermostBodyInSystem.name;
			}
		}
		...
		class DateofYearAchievement : Achievement
		{
			...
			public DateofYearAchievement(int firstDay, int firstMonth, int lastDay, int lastMonth, string name, string text,
				int prestige)
		...
		class WetEvaAchievement : Achievement
		{
			...
			public override string GetDescription()
			{
				return "Awarded for" + FirstKerbalText().Envelope() + (HasToBeFirst() ? "on " : "") +
					   "EVA in a wet environment outside of Kerbin";
			}
		}
		...
		class PassengerTransportAchievement : NumericAchievement
		{
			...
			protected override bool CheckVesselState(VesselState previous, VesselState current)
			{
				...
				if (previous == null ||
					(previous.Situation != Vessel.Situations.PRELAUNCH && previous.Situation != Vessel.Situations.LANDED))
					return false;
				if (current.Origin == null) return false;
				if (current.IsLanded) return false;
				if (current.IsEVA) return false;
		...
		class LostAndFoundAchievement : Achievement
		{
			...
			protected override bool CheckKerbalRosterStatus(ProtoCrewMember kerbal, ProtoCrewMember.RosterStatus oldState,
				ProtoCrewMember.RosterStatus newState)
			{
				if (oldState != ProtoCrewMember.RosterStatus.Dead && oldState != ProtoCrewMember.RosterStatus.Missing) return false;
				if (newState != ProtoCrewMember.RosterStatus.Available && newState != ProtoCrewMember.RosterStatus.Assigned)
					return false;
				return true;
			}
		...
		class PolarLandingAchievement : Achievement
		{
			...
			public PolarLandingAchievement(string hemisphere, int prestige, bool first)
				: base(
					"P" + hemisphere.Substring(0, 1).ToUpper() + (first ? "1" : ""),
					(first ? "First " : "") + hemisphere + "Polar Lander", prestige, first)
			{
			...
			public override string GetDescription()
			{
				return "Awarded for " + FirstKerbalText().Envelope() + "landing a vessel in the " + hemisphere +
					   " polar region off Kerbin";
			}
		}
	}
}