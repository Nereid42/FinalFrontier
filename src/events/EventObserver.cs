	...
	namespace FinalFrontier
	{
		class EventObserver
		{
			...
			private void OnStageActivate(int stage)
			{
				Vessel vessel = FlightGlobals.ActiveVessel;
				if (vessel == null) return;
				Log.Detail("stage " + stage + " activated for vessel " + vessel.name + " current mission time is " +
						   vessel.missionTime);
			}
			...
			private void OnGameStateCreated(Game game)
			{
				...
				Log.Info("EventObserver:: OnGameStateCreated " + game.UniversalTime + ", game status: " + game.Status + ", scene " +
			...
			private void CheckForLaunch(Vessel vessel, Vessel.Situations from, Vessel.Situations to)
			{
				if (from == Vessel.Situations.PRELAUNCH && to != Vessel.Situations.PRELAUNCH)
				{
					...
					{
						Log.Info("launch of vessel " + vessel.name + " at kerbin detected at " +
								 Utils.ConvertToKerbinTime(vessel.launchTime));
			...
			private void FireCustomEvents()
			{
				...
				if (vessel != null)
				{
					...
					{
						OnVesselSOIChanged(new GameEvents.HostedFromToAction<Vessel, CelestialBody>(vessel, currentSphereOfInfluence,
							vessel.mainBody));
					}
					// Orbit closed
				...
				// Orbit changed
				// gee force changed
				if (machInspector.StateHasChanged() || atmosphereInspector.StateHasChanged() || orbitInspector.StateHasChanged() ||
					geeForceStateChanged)
				{
					CheckAchievementsForVessel(vessel);
				}
			}
					...
					if (vessel != null)
					{
						...
						if (altitudeInspector.StateHasChanged())
						{
							if (Log.IsLogable(Log.LEVEL.DETAIL))
								Log.Detail("reset of mach and altitude inspecteurs because of change in altitude");
							machInspector.Reset();
							altitudeInspector.Reset();
						}
